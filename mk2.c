...
static int parse_buffered_data(struct mk2_read_buffer *read_buffer, char __user *user_buffer, size_t user_size)
{
	...
	while (rb_index < leftover) {
		...
		switch (read_buffer->data[rb_index] & 0x0f) {
			...
			default:
				// TODO: how to report parsing errors?
				return -EFAULT;
		}
		...
		// Metadata is telling us that we should have more data in the payload than we have.
		// It doesn't make any sense, something went horribly wrong.
		// TODO: close user file pointer and reset the device
...
retry:
	...
	if (leftover) {
		...
		// TODO: bring this feature of prefetching back
		//if (leftover < count)
